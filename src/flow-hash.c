...
Flow *FlowGetFlowFromHash(ThreadVars *tv, DecodeThreadVars *dtv, const Packet *p)
{
    ...
    if (FlowCompare(f, p) == 0) {
        ...
        while (f) {
            ...
            if (FlowCompare(f, p) != 0) {
                ...
                /* update the last seen timestamp of this flow */
                if (timercmp(&p->ts, &f->lastts, <)) {
                    SCLogNotice("Unordered packets %ld.%06ld vs %ld.%06ld on %p handled by %s", p->ts.tv_sec, p->ts.tv_usec, f->lastts.tv_sec, f->lastts.tv_usec, f, tv->name);
                } 
                COPY_TIMESTAMP(&p->ts,&f->lastts);
    ...
    /* update the last seen timestamp of this flow */
    if (timercmp(&p->ts, &f->lastts, <)) {
        SCLogDebug("Unordered packets %ld.%06ld vs %ld.%06ld on %p handled by %s (%d) rxhash: %u", p->ts.tv_sec, p->ts.tv_usec, f->lastts.tv_sec, f->lastts.tv_usec, f, tv->name, f->thread_id, p->rxhash);
        StatsIncr(tv, dtv->counter_flow_unordered_pkts);
    } 
    if (f->rxhash) {
        ...
        if (p->tcph && (! TCP_ISSET_FLAG_SYN(p))) {
            SCLogDebug("first packet, no syn");
            StatsIncr(tv, dtv->counter_flow_nosynfirst);
        }
        f->rxhash = p->rxhash;
    }
    COPY_TIMESTAMP(&p->ts,&f->lastts);
