...
#define FLOW_SGH_TOSERVER                 (1 << 11)
/** Sgh for toclient direction set (even if it's NULL) */
#define FLOW_SGH_TOCLIENT                 (1 << 12)

/** packet to server direction has been logged in drop file (only in IPS mode) */
#define FLOW_TOSERVER_DROP_LOGGED         (1 << 13)
/** packet to client direction has been logged in drop file (only in IPS mode) */
#define FLOW_TOCLIENT_DROP_LOGGED         (1 << 14)
/** alproto detect done.  Right now we need it only for udp */
#define FLOW_ALPROTO_DETECT_DONE          (1 << 15)
#define FLOW_NO_APPLAYER_INSPECTION       (1 << 16)

/** Pattern matcher alproto detection done */
#define FLOW_TS_PM_ALPROTO_DETECT_DONE    (1 << 17)
/** Probing parser alproto detection done */
#define FLOW_TS_PP_ALPROTO_DETECT_DONE    (1 << 18)
/** Pattern matcher alproto detection done */
#define FLOW_TC_PM_ALPROTO_DETECT_DONE    (1 << 20)
/** Probing parser alproto detection done */
#define FLOW_TC_PP_ALPROTO_DETECT_DONE    (1 << 21)
#define FLOW_TIMEOUT_REASSEMBLY_DONE      (1 << 23)
/** even if the flow has files, don't store 'm */
#define FLOW_FILE_NO_STORE_TS             (1 << 24)
#define FLOW_FILE_NO_STORE_TC             (1 << 25)

/** flow is ipv4 */
#define FLOW_IPV4                         (1 << 26)
/** flow is ipv6 */
#define FLOW_IPV6                         (1 << 27)

/** no md5 on files in this flow */
#define FLOW_FILE_NO_MD5_TS               (1 << 28)
#define FLOW_FILE_NO_MD5_TC               (1 << 29)

/** no size tracking of files in this flow */
#define FLOW_FILE_NO_SIZE_TS              (1 << 30)
#define FLOW_FILE_NO_SIZE_TC              (1 << 31)
