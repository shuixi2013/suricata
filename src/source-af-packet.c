...
/* FIXME reference count the block to free it */
...
static inline int AFPParsePacketV3(AFPThreadVars *ptv, struct block_desc *pbd, struct tpacket3_hdr *ppd)
{
    ...
    /* add forged header */ /* FIXME here */
    if (ptv->cooked) {
        SllHdr * hdrp = (SllHdr *)ptv->data;
        struct sockaddr_ll *from;
        from = (void *)ppd + TPACKET_ALIGN(ptv->tp_hdrlen);
...
int AFPReadFromRingV3(AFPThreadVars *ptv)
{
    ...
    while (1) {
        ...
        pbd = (struct block_desc *) ptv->rd[block_num].iov_base;
        ...
        if (unlikely(AFPWalkBlock(ptv, pbd, block_num) != AFP_READ_OK))
            break;

        AFPFlushBlock(pbd);
        block_num = (block_num + 1) % ptv->req3.tp_block_nr;
    }
...
static int AFPComputeRingParamsV3(AFPThreadVars *ptv, int order)
{
    ptv->req3.tp_block_size = getpagesize() << order;
    ptv->req3.tp_frame_size = ptv->req3.tp_block_size / 2; /* FIXME Why ? */

    if (ptv->req3.tp_frame_size == 0) {
        SCLogInfo("frame size is 0");
        return -1;
    }
    ...
    ptv->req3.tp_retire_blk_tov = 60; /* FIXME Why 60 ? */
    ptv->req3.tp_feature_req_word = TP_FT_REQ_FILL_RXHASH;
    SCLogInfo("AF_PACKET V3 RX Ring params: block_size=%d block_nr=%d frame_size=%d frame_nr=%d",
...
#define DEFAULT_ORDER 3
        for (order = DEFAULT_ORDER; order >= 0; order--) {
            if (ptv->flags & AFP_TPACKET_V3) {
                if (AFPComputeRingParamsV3(ptv, order) != 1) {
        ...
        if (ptv->flags & AFP_TPACKET_V3) {
            ptv->rd = malloc(ptv->req3.tp_block_nr * sizeof(*ptv->rd));
            if (!ptv->rd) {
